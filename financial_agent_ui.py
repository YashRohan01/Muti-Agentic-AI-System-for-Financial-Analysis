...
def render_concise_answer(query,bullet_points=None):
    """Render the concise answer section"""
    st.markdown(f"#### 🔍 Query Asked: {query}")
...
def render_generated_code(code_content):
    """Render the generated code section with copy functionality"""
    
    # Create the code container with custom styling
    st.markdown("""
    <div class="code-container" style="border: 1px solid #ccc; padding: 10px; border-radius: 10px; background-color: #f9f9f9;">
        <div class="code-header" style="display: flex; align-items: center;">
            ...
            <h3 style="margin: 0;">Final Python Code Generated for the Report</h3>
        </div>
    </div>
    """, unsafe_allow_html=True)

    
    # Display the code with syntax highlighting and copy button
    if code_content:
        ...
        # Display the code
        st.code(clean_code, language="python", line_numbers=True)
        ...
        <div class="code-container">
            <div style="text-align: center; color: #6C757D; font-style: italic; padding: 40px 0;">
                No code was generated for this query
            </div>
        </div>
        """, unsafe_allow_html=True)

def render_report_warning():
    """Render warning when no report is generated"""
    st.markdown("""
    <div class="warning-container">
        <div class="warning-icon">⚠️</div>
        <div class="warning-content">
            <h4 class="warning-title">Report Generation Failed</h4>
            <p class="warning-text">
                The AI agents completed the analysis but were unable to generate a PDF report. 
                This might be due to system limitations or the nature of your query. 
                Please check the concise answer and generated code sections above for the analysis results.
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def run_analysis_with_thinking_process(query, thinking_container):
    ...
    # Initialize generated_code to track the final code
    generated_code = None
    ...
    thinking_steps.append(f"<b>Query Type</b>: {query_response.type}")
    thinking_steps.append(f"<b>Explanation</b>: {query_response.explanation}")
    update_thinking_display()
    ...
    if query_response.type == 'simple':
        ...
        generated_code = verified_code  # Store the generated code
        ...
        thinking_steps.append(f"<b>Tasks</b>: {', '.join(task_response.tasks)}")
        thinking_steps.append(f"<b>Companies</b>: {', '.join(task_response.companies)}")
        thinking_steps.append(f"<b>CIKs</b>: {', '.join(map(str, task_response.ciks))}")
        thinking_steps.append(f"<b>Metrics</b>: {', '.join(task_response.metrics)}")
        update_thinking_display()
        ...
        if 'financial_metric_analysis' in task_response.tasks:
            ...
            thinking_steps.append(f"<b>Requires Visualization<b>: {financial_analysis_response.require_visualization}")
            update_thinking_display()
            ...
            thinking_steps.append(f"<b>Requires Performance Summary</b>: {financial_analysis_response.require_summarization}")
            update_thinking_display()            
        ...
        if 'narrative_analysis' in task_response.tasks:
            ...
            thinking_steps.append(f"<b>Requires Strategic Recommendations</b>: {narrative_analysis_response.require_strategic_recommendation}")
            update_thinking_display()
            ...
            thinking_steps.append(f"<b>Requires Risk Analysis</b>: {narrative_analysis_response.require_risk_analysis}")
            update_thinking_display()
        ...
        if 'predictive_analysis' in task_response.tasks:
            ...
            future_datas,simple_future_data = process_and_forecast(datas)
        ...
        brief_answer = concise_answer_chain.invoke({
            ...
            'performance_analysis': getattr(performance_summary_response, 'performance_report', "Performance analysis not required."),
            'narrative_analysis': getattr(narrative_analysis_response, 'narrative_report', "Narrative analysis not required."),
            'strategic_recommendations': getattr(strategic_reccomendation_response, 'recommendation_report', "Recommendations not required.") if strategic_reccomendation_response else "Recommendations not required.",
            'risk_analysis': getattr(risk_analysis_response, 'risk_analysis_report', "Risk Analysis not required.") if risk_analysis_response else "Risk Analysis not required.",
            'future_datas': simple_future_data or {},
    ...
    return thinking_steps, brief_answer, generated_code
...
""", unsafe_allow_html=True)
    ...
    <ul>
        ...
        <li>How does Meta's approach to sustainability reporting align with industry best practices, and what measurable outcomes have been achieved in environmental, social, and governance (ESG) areas over the past three years?</li>
        ...
        <li>How has Meta's and Apple's debt profile changed over the last five years?</li>
        <li>How has Apple's business strategy evolved over the last five years, particularly in response to competitive pressures and technological changes?</li>
        ...
        <li>How has Meta's ESG changed over the last 3 years?</li>
    </ul>
    """, unsafe_allow_html=True)
    ...
    if analyze_button and query:
        ...
        with st.spinner("🤖 AI agents are working on your query..."):
            thinking_steps, brief_answer, final_code = run_analysis_with_thinking_process(query, thinking_container)
        ...
        if brief_answer and hasattr(brief_answer, 'points'):
            render_concise_answer(query,brief_answer.points)
        else:
        ...
        render_generated_code(final_code)
...
def architecture_page():
    ...
    agents = [
        {
        "Agent Name": "Query Classification Agent",
        "Purpose": "Determines query complexity and routing",
        "Input": "Query",
        "Output": "simple / complex"
    },
    {
        "Agent Name": "RAG Agent",
        "Purpose": "Retrieval-Augmented Generation for document processing",
        "Input": "Query, Relevant Documents",
        "Output": "Summary."
    },
    {
        "Agent Name": "Task Allocator",
        "Purpose": "Decide what tasks are required to answer the query from narrative_analysis, financial_analysis and predictive_analysis",
        "Input":"query",
        "Output": "tasks, companies, ciks, metrics and explanation"
    },
    {
        "Agent Name": "Financial Analysis Agent",
        "Purpose": "Quantitative financial data analysis",
        "Input": "Query, Available Metrics",
        "Output": "Metric Analysis, visualizations?, performance summary?"
    },
    {
        "Agent Name": "Visualization Agent",
        "Purpose": "Chart and graph generation",
        "Input": "Processed Financial Data from dataframe",
        "Output": "Matplotlib Images"
    },
    {
        "Agent Name": "Performance Analysis Agent",
        "Purpose": "Analyze the performance",
        "Input": "Processed Financial Data from dataframe",
        "Output": "Performance Summary"
    },
    {
        "Agent Name": "Narrative Analysis Agent",
        "Purpose": "10K document interpretation",
        "Input": "Query, Relevant 10K documents",
        "Output": "Textual Insights, risk analysis?, strategic recommendations?"
    },
    {
        "Agent Name": "Strategic Recommendation Agent",
        "Purpose": "Strategic Recommendations from 10K document interpretation",
        "Input": "Query, Relevant 10K documents,Textual Insights",
        "Output": "Actionable Strategies"
    },
    {
        "Agent Name": "Risk Analysis Agent",
        "Purpose": "Risk factor identification and assessment",
        "Input": "Query, 10-K Risk Sections, Textual Insights",
        "Output": "Key Risks and Evaluations"
    },
    {
        "Agent Name": "Holt Linear Prediction Agent",
        "Purpose": "Time series forecasting and predictions",
        "Input": "Historical Financial Metrics",
        "Output": "Forecasted Values"
    },
    {
        "Agent Name": "ARIMA Prediction Agent",
        "Purpose": "Time series forecasting and predictions",
        "Input": "Historical Financial Metrics",
        "Output": "Forecasted Values"
    },
    {
        "Agent Name": "Report Writing Agent",
        "Purpose": "Comprehensive report compilation",
        "Input": "All Analysis Outputs",
        "Output": "Structured PDF Report"
    },
    {
        "Agent Name": "Code Verification Agent",
        "Purpose": "Code validation and execution",
        "Input": "Generated Code",
        "Output": "Execution Result or Errors"
    }
    ]

    df = pd.DataFrame(agents)

    st.markdown("### 🤖 Agent Overview Table")
